...
test('parses valid JSON response and marks first frame as changed', async () => {
  const visionClient = { chat: async () => ({ content: '{"description":"a"}', latencyMs: 42 }) };
...
test('falls back to raw text when response is not valid JSON', async () => {
  const visionClient = { chat: async () => ({ content: 'not json', latencyMs: 5 }) };
  const stateHistory = new StateHistory({ historyDepth: 5, historyImages: 0 });
  const engine = new InferenceEngine({ visionClient, promptLoader: fakePromptLoader(), stateHistory });
...
test('skips history push and emits inferenceError on visionClient failure', async () => {
  const visionClient = { chat: async () => { throw new Error('boom'); } };
  const stateHistory = new StateHistory({ historyDepth: 5, historyImages: 0 });
  const engine = new InferenceEngine({ visionClient, promptLoader: fakePromptLoader(), stateHistory });
